local ontrawl = function(player, data)
	...
	if not krakener then
		return
	end

    if math.random() <= krakener.spawn_chance then
		if krakener:CheckTileCompatibility(player) then
			krakener:DoKrakenEvent(nil, player)
		end
	end
end
...
local Krakener = Class(function(self, inst)
	...
	self.valid_tiles = table.invert({WORLD_TILES.OCEAN_DEEP, WORLD_TILES.OCEAN_SHIPGRAVEYARD})

	self.inst:ListenForEvent("ms_playerjoined", function (src, player)
		player:ListenForEvent("trawlitem", ontrawl)
	end)
end)

function Krakener:DoKrakenEvent(force, player)
	local spawnpt = self:GetNearbySpawnPoint(player)
...
function Krakener:LongUpdate(dt)
...
function Krakener:CheckTileCompatibility(x, y, z)  -- can passed inst or x,y,z or Vector3
	return CheckTileAtPoint(x, y, z, self.valid_tiles)
end
...
function Krakener:GetNearbySpawnPoint(inst)
	local width, height = TheWorld.Map:GetWorldSize()
	local pos = inst:GetPosition()
	local new_pos = pos

	for i = 1, 500 do
		local offset = FindSwimmableOffset(pos, math.random() * 2 * math.pi, 40, 12)
		if offset then
			new_pos = pos + offset
			local x, y = TheWorld.Map:GetTileCoordsAtPoint(new_pos:Get())
			if self:CheckTileCompatibility(new_pos) and GetDistFromEdge(x, y, width, height) >= 15 then
				break
			end
		end
	end
...
function Krakener:SetCooldownModifier(respawn)
	...
	self.respawn_time_mod = respawn or 1.0
end
