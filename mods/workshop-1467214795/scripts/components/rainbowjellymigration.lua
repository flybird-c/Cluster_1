...
RainbowJellyfishMigrationManager = Class(function(self, inst)
    ...
    self.inst:WatchWorldState("cycles", function(inst, data) self:OnDayComplete() end )
end)
...
local function setupHomeAndMigrationDestination(jelly, migrationPos, teleport)
    ...
    local offset = FindSwimmableOffset(migrationPos, math.random() * 2 * PI, math.random(2,25), 4) or Vector3(0,0,0)
...
function RainbowJellyfishMigrationManager:StartMigration()
    ...
    if self.isMigrationActive == true or theVolcano == nil or not theVolcano:IsValid() then
        return
    end
