...
IA_VEGGIES = {
    sweet_potato = MakeVegStats(COMMON, TUNING.CALORIES_SMALL, TUNING.HEALING_TINY, TUNING.PERISH_MED, 0, TUNING.CALORIES_SMALL, TUNING.HEALING_SMALL, TUNING.PERISH_FAST, 0)
}
...
local assets_seeds = {
    ...
    Asset("ANIM", "anim/ia_farm_plant_seeds.zip"),
}
...
local function MakeVeggie(name, has_seeds)
    ...
    if IA_VEGGIES[name].lure_data ~= nil then
        table.insert(assets, Asset("ANIM", "anim/" .. IA_VEGGIES[name].lure_data.build .. ".zip"))
    end
    ...
    local dryable = IA_VEGGIES[name].dryable
    ...
    local function fn()
        ...
        local float = IA_VEGGIES[name].float_settings
        if float ~= nil then
        ...
        inst.components.edible.secondaryfoodtype = IA_VEGGIES[name].secondary_foodtype
		...
		if IA_VEGGIES[name].lure_data ~= nil then
			inst:AddComponent("oceanfishingtackle")
			inst.components.oceanfishingtackle:SetupLure(IA_VEGGIES[name].lure_data)
		end

		local halloweenmoonmutable_settings = IA_VEGGIES[name].halloweenmoonmutable_settings
    ...
    local function fn_cooked()
        ...
        local float = IA_VEGGIES[name].cooked_float_settings
        if float ~= nil then
            MakeInventoryFloatable(inst, float[1], float[2], float[3])
        else
            MakeInventoryFloatable(inst)
    ...
    local function fn_oversized()
        ...
        inst:AddComponent("perishable")
        inst.components.perishable:SetPerishTime(IA_VEGGIES[name].perishtime * OVERSIZED_PERISHTIME_MULT)
        inst.components.perishable:StartPerishing()
...
for veggiename, veggiedata in pairs(IA_VEGGIES) do
    ...
    table.insert(Prefabs.plant_normal.assets, Asset("ANIM", "anim/" .. veggiename))
    table.insert(Prefabs.plant_normal.deps, veggiename)
    table.insert(Prefabs.seeds.deps, veggiename)
