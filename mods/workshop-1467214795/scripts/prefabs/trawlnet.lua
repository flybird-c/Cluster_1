...
local function getLootList(inst)
    ...
    if owner and owner.components.sailable and owner.components.sailable.sailor then
        ...
        pos = owner:GetPosition() or inst:GetPosition()
    end
	
    local depth = GetTileDepth(TheWorld.Map:GetTileAtPoint(pos:Get()))

    if depth >= OCEAN_DEPTH.VERY_DEEP then
        return loottable.deep
    elseif depth >= OCEAN_DEPTH.DEEP then
