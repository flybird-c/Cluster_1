...
local function get_wind_anims(inst, type)
    if type == 1 then
        ...
        return anims[inst.statedata.name]["blown" .. tostring(anim)]
    elseif type == 2 then
        return anims[inst.statedata.name].blown_pst
    end
    return anims[inst.statedata.name].blown_pre
end

local function can_play_wind_anim(inst)
    return anims[inst.statedata.name] ~= nil
end
...
local function palmtree_common_postinit_fn(inst)
    inst.AnimState:AddOverrideBuild("coconut")
end

local function palmtree_onchop(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
    SpawnPrefab("pine_needles_chop").Transform:SetPosition(x, y + 2 + math.random() * 2, z)
end

-------------------------------------------------------------------------------

local function jungletree_common_postinit_fn(inst)
    inst.AnimState:AddOverrideBuild("jungletreeseed")
end

local function jungletree_onchop(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
    SpawnPrefab("jungle_chop").Transform:SetPosition(x,y + 2 + math.random()*2,z)
end
...
local function AddWinterTree(treetype)
    ...
    if treetype.extrabuilds ~= nil then
        for _, v in ipairs(treetype.extrabuilds) do
    ...
    for _, v in ipairs(GetAllWinterOrnamentPrefabs()) do
    ...
    if treetype.extraprefabs ~= nil then
        for _, v in ipairs(treetype.extraprefabs) do
            table.insert(prefabs, v)
        end
    end
    for k in pairs(random_gift1) do
        table.insert(prefabs, k)
    end
    for k in pairs(random_gift2) do
    ...
    local function fn()
        ...
        inst.MiniMapEntity:SetIcon(treetype.name .. ".tex")
        ...
        if treetype.blowinwindgust ~= nil then
            MakeTreeBlowInWindGust(inst, treetype.blowinwindgust.wind_speed, treetype.blowinwindgust.destroy_chance)
...
local ia_wintertree_data = {
    {
        name = "winter_palmtree",
        bank = "wintertree_ia_palm",
        build = "ia_palmtree_build",
        seedprefab = "coconut",
        extrabuilds = {
            "coconut",
        },
        -- blowinwindgust = {
        --     wind_speed = TUNING.PALMTREE_WINDBLOWN_SPEED,
        --     destroy_chance = 0,
        -- },
        onchop = palmtree_onchop,
        shelter = true,  -- dynamic
        common_postinit = palmtree_common_postinit_fn,
    },
    {
        name = "winter_jungletree",
        bank = "wintertree_jungle",
        build = "tree_jungle_build",
        seedprefab = "jungletreeseed",
        extrabuilds = {
            "jungletreeseed",
        },
        -- blowinwindgust = {
        --     wind_speed = TUNING.JUNGLETREE_WINDBLOWN_SPEED,
        --     destroy_chance = 0,
        -- },
        onchop = jungletree_onchop,
        shelter = true,  -- dynamic
        common_postinit = jungletree_common_postinit_fn,
    },
}

for _, data in ipairs(ia_wintertree_data) do
    AddWinterTree(data)
end
