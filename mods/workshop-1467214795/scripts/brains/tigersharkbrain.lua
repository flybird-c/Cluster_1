...
local function GetWanderPoint(inst)
    -- if inst.components.knownlocations and inst.components.knownlocations:GetLocation("point_of_interest") then --This code doesnt work in ds because of a typo so it never runs -Half
    --     return inst.components.knownlocations:GetLocation("point_of_interest")
    -- end

    local shark_home = inst:FindSharkHome()
    if shark_home and inst:GetPosition():Dist(shark_home:GetPosition()) < 40 then
        return shark_home:GetPosition()
    end
...
local function FindWaterAction(inst)
    ...
    local wateroffset = FindSwimmableOffset(inst:GetPosition(), math.random() * 2 * math.pi, 30, 36)
    if wateroffset then
...
function TigersharkBrain:OnStart()
    local root = PriorityNode(
    {
        ...
        --Run home
        WhileNode(function() return self.inst:FindSharkHome() and self.inst.components.health:GetPercent() <= 0.1 end, "Low Health",
            PriorityNode({
        ...
        IfNode(function() return self.inst:FindSharkHome() end, "Has Home",
