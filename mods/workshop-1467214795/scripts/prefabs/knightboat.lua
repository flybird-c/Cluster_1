...
local RETARGET_CANT_TAGS = {"FX", "NOCLICK","INLIMBO", "CLASSIFIED", "chess"}
local RETARGET_NO_TAGS = {"character", "monster"}

local function RetargetFn(inst)
    local newtarget = FindEntity(inst, 25, function(guy)
            if not guy:IsNear(inst, 25 * 0.5) and guy:HasTag("chessfriend") then
                return false
            end
            return inst.components.combat:CanTarget(guy)
    end, nil, RETARGET_CANT_TAGS, RETARGET_NO_TAGS)
    return newtarget
end
...
local function onsave(inst, data)
    data.raider = inst.raider
end

local function onload(inst, data)
    if data ~= nil and data.raider then
        inst.raider = data.raider
    end
end
