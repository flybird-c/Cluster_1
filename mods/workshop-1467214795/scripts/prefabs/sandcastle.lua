...
local function onupdate(inst)
	...
	local wind = ((inst.components.whirlspeed_adjustable and inst.components.whirlspeed_adjustable:IsSpeedAdjusted() and (TheWorld.state.gustspeed * TUNING.SAILSTICK_BONUSSPEEDMULT)) or TheWorld.state.gustspeed)
...
local function getstatus(inst, viewer)
  ...
  if TheWorld.Map:GetTileAtPoint(x, y, z) == WORLD_TILES.BEACH then
    return "SAND"
  else
    return "GENERIC"
  end
...
--allow the sailstick to modify the windspeed if nearby
  inst:AddComponent("whirlspeed_adjustable")
