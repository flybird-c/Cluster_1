...
local function GenerateBermudaTriangles(root, entities, width, height)
    ...
    local pointsX, pointsY = SpawnUtil.FindRandomWaterPoints(checkTriangle, width, height, TUNING.MAPEDGE_PADDING, numTriangles)
    ...
    for i = #entities.bermudatriangle_MARKER, 1, -1 do
        ...
        if firstMarkerData ~= nil then
            ...
            if IAENV.max_pairs and pair >= IAENV.max_pairs then
                break
            end
        end
    end
...
local function GenerateTreasure(root, entities, width, height)
	...
	local pointsX_g, pointsY_g = SpawnUtil.FindRandomWaterPoints(checkLand, width, height, TUNING.MAPEDGE_PADDING, numTreasures)
	local pointsX_w, pointsY_w = SpawnUtil.FindRandomWaterPoints(checkWater, width, height, TUNING.MAPEDGE_PADDING, numBottles)
...
Graph.GlobalPostPopulate = function(self, entities, width, height, ...)
	...
	if IA_worldtype ~= "default" and self.isshipwrecked then
