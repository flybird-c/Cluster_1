...
NOTE: target must be on water to spawn shark.
--]]
...
function TigerSharker:OnEvent(event, doer)
    ...
    if self.events[event] and IsInIAClimate(doer) and doer:IsOnOcean() then
...
function TigerSharker:OnAction(action, doer)
    --If we're listening for this and the doer is on the water.
    if self.actions[action.action] and IsInIAClimate(doer) and doer:IsOnOcean() then
        --Roll against the chance for this event to trigger a spawn.
...
function TigerSharker:GetNearbySpawnPoint(target)
    ...
    if to_home then
        ...
        local offset = FindSwimmableOffset(unpack(properties))
...
local SHARKEVENT_CANT_TAGS = {"prey", "player", "companion", "bird", "butterfly", "sharkitten"}
function TigerSharker:DoSharkEvent(target)
    ...
    if spawnpt then
        ...
        if shark then
            ...
            -- local home_pos = self:GetHomePosition()
            -- shark.components.knownlocations:RememberLocation("point_owaf_interest", home_pos) --Typo Yay! -Half
...
function TigerSharker:SpawnShark(ignore_cooldown, ignore_death)
    if not self:CanSpawn(ignore_cooldown, ignore_death) then
        return
    end
