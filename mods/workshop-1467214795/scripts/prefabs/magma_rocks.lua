...
local function Getgrowtime(rock)
    local day = TUNING.TOTAL_DAY_TIME * 15
    local volcano = IsInClimate(rock, "volcano")
    if volcano and TheWorld.state.issummer then
        day = TUNING.TOTAL_DAY_TIME * 5
    elseif volcano or TheWorld.state.issummer then
        day = TUNING.TOTAL_DAY_TIME * 10
    end
    return day
end
...
local function onworked(inst, worker, workleft)
    ...
    if nextStage ~= inst.stage then
        ...
        inst.components.timer:StartTimer("grow", Getgrowtime(inst))
        end
    end
end
...
local function OnSummer(inst)
    local timeleft = inst.components.timer:GetTimeLeft("grow")
    local season_time = (IsInClimate(inst, "volcano") and 5 or 10)
    if timeleft and timeleft > TUNING.TOTAL_DAY_TIME * season_time then
        inst.components.timer:SetTimeLeft("grow", TUNING.TOTAL_DAY_TIME * season_time)
    end
end
...
local function commonfn()
    ...
    inst:WatchWorldState("issummer", OnSummer, TheWorld)
    inst.OnLoad = OnLoad
    inst.OnSave = OnSave
