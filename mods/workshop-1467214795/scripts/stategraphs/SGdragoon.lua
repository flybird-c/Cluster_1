...
local actionhandlers =
{
	...
	ActionHandler(ACTIONS.DRAGOON_LAVASPIT, "spit"),
...
local states=
{
	...
	State{
		...
		onenter = function(inst, playanim)
			inst.SoundEmitter:PlaySound("ia/creatures/dragoon/idle")
			inst.Physics:Stop()
			if playanim then
	...
	State{
		...
		onenter = function(inst, target)
			...
			inst.SoundEmitter:PlaySound("ia/creatures/dragoon/attack")
		end,
		...
		{   
			...
			TimeEvent(15*FRAMES, function(inst) 
				...
				inst.SoundEmitter:PlaySound("ia/creatures/dragoon/attack_strike")
			end),
	...
	State{
		...
		onenter = function(inst)
			...
			if ((inst.target ~= inst and not inst.target:HasTag("fire")) or inst.target == inst) and not (inst.recently_frozen) then
				...
				inst.SoundEmitter:PlaySound("ia/creatures/dragoon/hork")
			else
				-- print("no spit")
		...
		timeline=
		{
			TimeEvent(37*FRAMES, function(inst) 
				...
				inst.SoundEmitter:PlaySound("ia/creatures/dragoon/spit")
			...
			TimeEvent(39*FRAMES, function(inst) 
				-- print("spit timeline")
				-- print("vomitfire_fx spawned")
				inst.SoundEmitter:PlaySound("ia/creatures/dragoon/fireball")
			end),
		},
	},
	...
	State{
		...
		onenter = function(inst, cb)
			...
			inst.SoundEmitter:PlaySound("ia/creatures/dragoon/hit")
		end,
	...
	-- 		inst.SoundEmitter:PlaySound("ia/creatures/dragoon/taunt")
	-- 	end,
	...
	State{
		...
		onenter = function(inst)
			inst.SoundEmitter:PlaySound("ia/creatures/dragoon/death")
	...
	State{
		...
		timeline = {
			--TimeEvent(0, function(inst) inst.SoundEmitter:PlaySound("ia/creatures/snake/taunt") end),
	...
	State{
		...
		onenter = function(inst)
			...
			inst.SoundEmitter:PlaySound("ia/creatures/dragoon/charge")
...
{
	sleeptimeline = {
		TimeEvent(30*FRAMES, function(inst) inst.SoundEmitter:PlaySound("ia/creatures/dragoon/sleep") end),
	},
})
CommonStates.AddFrozenStates(states)
CommonStates.AddSimpleActionState(states, "gohome", "hit", 1*FRAMES, {"busy"})
CommonStates.AddHopStates(states, false, {pre = "walk_pre", loop = "walk_loop", pst = "walk_pst"})
