...
local function LavaSpitAction(inst)
    ...
    if not inst.target or (inst.target ~= inst and not inst.target:HasTag("fire")) then
        ...
        return BufferedAction(inst, inst.target, ACTIONS.DRAGOON_LAVASPIT)
    end
end

local function FindLavaSpitTargetAction(inst)
    if inst.sg:HasStateTag("sleeping") or inst.num_targets_vomited >= DRAGONFLY_VOMIT_TARGETS_FOR_SATISFIED or inst.hassleepdestination then
        return false
    end

    if inst.last_spit_time and ((GetTime() - inst.last_spit_time) < 5) then
        return false
    end
