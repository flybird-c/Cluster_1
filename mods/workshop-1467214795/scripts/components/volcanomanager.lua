...
local new_players = {} -- Used to protect new players
local function OnPlayerJoined(src, player)
	if LastMigrator == player then
		return
	end
    for i, v in ipairs(new_players) do
        if v == player then
            return
        end
    end
    table.insert(new_players, player)
	src:DoTaskInTime(120, function(_src)  -- protect new players 2 min
		table.remove(new_players, 1)
	end)
end
...
function VolcanoManager:DoWarnQuake(duration, speed, scale)
	duration = duration and duration > 0  and duration or WARN_QUAKE_DURATION
	speed = speed and speed > 0 and speed or 0.02
	scale = scale and scale > 0 and scale or .75
...
function VolcanoManager:DoEruption(data)
	...
	for _, player in pairs(AllPlayers) do
		if IsInIAClimate(player) and not table.contains(new_players, player) then
			player.player_classified.volcanoeruption:set_local(false)
			player.player_classified.volcanoeruption:set(true)
		end
	end
end
...
function VolcanoManager:RebuildDrySchedule()
	if self.intensity > 0.0 then
		local volcanoschedule = deepcopy(VolcanoSchedule)
...
function VolcanoManager:StartStaffTrap()
	...
	--self:DoWarnQuake()
	self:ResumeStaffTrap( math.floor( 16 * (TheWorld.state.cycles + TheWorld.state.time) ) )
end
		...
		for seg, _ in pairs(schedule) do
			if seg <= segs then
				for i = 1, #schedule[seg], 1 do
					schedule[seg][i].fn(self, schedule[seg][i].data)
				end
				schedule[seg] = nil
			end
		end
...
function VolcanoManager:GetCurrentScheduleSegment()
	local cycles = TheWorld.state.seasonprogress * TheWorld.state.summerlength
	local normTime = TheWorld.state.time
	return math.floor(16 * (cycles + normTime)) - self.appeasesegs
end

function VolcanoManager:GetCurrentStaffScheduleSegment()
	return math.floor( 16 * (TheWorld.state.cycles + TheWorld.state.time) ) - self.staffstartseg
end
...
function VolcanoManager:GetDebugString()
	...
	if self.schedule[1] then
		str = str .. string.format("  dry segs %d, season %4.2f\n", self:GetCurrentScheduleSegment(), TheWorld.state.seasonprogress)
	end
...
function VolcanoManager:OnUpdate( dt )
	...
	if self.schedule[1] then
		if TheWorld.state.issummer then
			...
			self:RebuildWarning(self.schedule[1], segs)
			if self.schedule[1] and IsTableEmpty(self.schedule[1]) then
				self:AddNewEruption(self.schedule[1], segs)
			end
	...
	--update ash rain
	local particles_per_tick = 0
	if self.ash_timer > 0.0 then
		self.ash_timer = self.ash_timer - dt
		if self.ash_timer <= self.ash_duration then
			particles_per_tick = 20 * math.min(self.ash_timer / self.ash_duration, 1.0)
			for _, player in pairs(AllPlayers) do
				if IsInIAClimate(player) then
					player.player_classified.ashfxparticles:set(particles_per_tick)
				end
			end
		else
			particles_per_tick = 0
			for _, player in pairs(AllPlayers) do
				if IsInIAClimate(player) then
					player.player_classified.ashfxparticles:set(particles_per_tick)
				end
	...
	--[[
	if GetInteriorSpawner():IsPlayerConsideredInside() then  --是否在室内(哈姆雷特房子)
		if self.ash then
			self.ash.particles_per_tick = 0
		end
	end
	--]]
    local smokerate = self:GetSmokeRate()
    for _, player in pairs(AllPlayers) do
		if IsInIAClimate(player) then
        	player.player_classified.smokerate:set(smokerate)
		end
    end
	...
	if self.waserupting and not self:IsErupting() then
		...
		for _, player in pairs(AllPlayers) do
			if IsInIAClimate(player) then
				player.player_classified.volcanoeruption:set_local(false)
				player.player_classified.volcanoeruption:set(false)
			end
		end
	end
...
function VolcanoManager:SetActiveIcon()
	for k, v in pairs(self.volcanoes) do
		if v.MiniMapEntity then
			v.MiniMapEntity:SetIcon("volcano_active.tex")
		end
	end
end
...
function VolcanoManager:SetDormantIcon()
	for k, v in pairs(self.volcanoes) do
		if v.MiniMapEntity then
			v.MiniMapEntity:SetIcon("volcano.tex")
		end
	end
end

function VolcanoManager:GetClosestVolcano(player)
	local closest = nil
...
function VolcanoManager:GetDistanceFromVolcano(player)
...
function VolcanoManager:GetSmokeRate()
	--[[
	if GetInteriorSpawner():IsPlayerConsideredInside() then  --是否在室内
    	return 0.0
	end
	--]]
...
function VolcanoManager:RebuildWarning(schedule, segs)
	if schedule then
		local dry, dryev = segs_until_event(DoEruption, schedule, segs)
		if dry and dryev then
			local eruption_seg = dry + segs
			for _, v in ipairs(VolcanoSchedule.DrySeasonSchedule[dryev.data.num].warnquake) do
				local quake_seg = eruption_seg - v.segsprev
				if quake_seg > segs and not schedule[quake_seg] then
					schedule[quake_seg] = {}
					table.insert(schedule[quake_seg], { fn = DoWarnQuake, data = {size = v.size} })
				end
			end
		end
	end
...
function VolcanoManager:AddNewEruption(schedule, start_segs)
	if schedule then
		local erupt = VolcanoSchedule.DrySeasonSchedule[17]
		...
		data.smoke_delay = (erupt.data.smoke_delay or 0) * seg_time
		data.smoke_duration = (erupt.data.smoke_duration or 1) * seg_time
		data.total_segs = 16 * (erupt.days or 0) + (erupt.segs or 0)
		data.num = erupt.data.num or 1

		schedule[segs] = {}
		table.insert(schedule[segs], {fn = DoEruption, data = data})
		self.schedulesegs[1] = segs
	end
end
