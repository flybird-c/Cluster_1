...
{   
	...
	State{
		...
		onenter = function(inst)
			...
			if IsSpecialEventActive(SPECIAL_EVENTS.WINTERS_FEAST) and math.random() < TUNING.DOYDOY_MATING_ORNAMENT_CHANCE then
				SpawnPrefab("winter_ornament_boss_doydoy").Transform:SetPosition(inst:GetPosition():Get())
			end
...
CommonStates.AddHopStates(states, true, {pre = "mate_pre", loop = "mate_dance_loop", pst = "mate_dance_pst"})
