...
local function PlaceBoatOnShore(inst, prefab)
	...
	local shorex, shorey = findtileradius(function(tile) return tile == WORLD_TILES.BEACH end, px, py, pz, 50)
	if shorex and shorey then
		...
		if boat then
			...
			local shalx, shaly = findtileradius(function(tile) return tile == WORLD_TILES.OCEAN_SHALLOW end, tx, 0, tz, 1)
			--local landx, landy = findtileradius(function(tile) return world.Map:IsLandTile(tile) end, tx, 0, tz, 1)
