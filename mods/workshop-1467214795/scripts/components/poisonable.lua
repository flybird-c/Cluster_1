...
function Poisonable:Poison(is_gas, load_time, strength)
    if load_time or self:CanBePoisoned(is_gas) then
        self.severity = strength and math.max(strength, self.severity) or self.severity
        self.inst:AddTag("poison")
        self.poisoned = true
        self.start_time = load_time and (GetTime() - load_time) or GetTime()
        ...
        if self.inst.components.areapoisoner and self.duration > 0 then
            self.inst.components.areapoisoner:StartSpreading(load_time and -load_time or self.duration)
        end
...
function Poisonable:OnLoad(data)
    if data.poisoned and data.poisontimeelapsed and IA_CONFIG.poisonenabled then
