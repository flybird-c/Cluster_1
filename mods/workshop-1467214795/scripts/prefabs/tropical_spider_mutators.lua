...
local assets =
{
    Asset("ANIM", "anim/mutator_tropical_spider_warrior.zip"),
...
{
    ["tropical_spider_warrior"] = {
        bank = "mutator_tropical_spider_warrior",
        build = "mutator_tropical_spider_warrior",
        playanim = "idle"
    },
}
...
local function MakeMutatorFn(mutator_target, extra_data)
    ...
    inst.AnimState:PlayAnimation((extra_data and extra_data.playanim) or mutator_target)
    ...
    inst.components.spidermutator:SetMutationTarget(mutator_target)
...
for i, mutator_target in ipairs(mutator_targets) do
    table.insert(prefabs, mutator_target)
end
