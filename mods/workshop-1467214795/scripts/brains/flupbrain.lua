...
local function SetUpAmbush(inst)
    if inst.components.combat.target or inst.sg:HasStateTag("ambusher") or inst:GetCurrentPlatform() ~= nil then
        return
    end
...
function FlupBrain:OnStart()
    local root = PriorityNode(
    {
        ...
        WhileNode(function() return not self.inst.components.combat.target and self.inst:GetCurrentPlatform() == nil end, "No Target",
