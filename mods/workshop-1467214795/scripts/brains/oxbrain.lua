...
function OxBrain:OnStart()
    local root = PriorityNode(
    {
        ...
        -- Removed in the DS beta but that ended up causing some other bugs and after discussing with leon (Diogo) he said to re add it -Half
        -- According to leon (Diogo) it was originally removed to fix a bug that was also fixed through a different method (so removing this is pointless)
        WhileNode( function() 
            local tile = self.inst.components.tiletracker.tile
            return tile ~= nil and IsOceanTile(tile) and tile ~= WORLD_TILES.MANGROVE
        end, "intheocean",  
