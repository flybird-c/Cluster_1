...
local function itemfn()
    local Pump = function(inst)
        inst.SoundEmitter:PlaySound("ia/creatures/dragoon/heart")

        if inst.components.floater:IsFloating() then
            inst.pumptask = inst:DoTaskInTime(inst.AnimState:GetCurrentAnimationLength("idle_water"), inst.pumpfn)
        else
            inst.pumptask = inst:DoTaskInTime(inst.AnimState:GetCurrentAnimationLength("idle"), inst.pumpfn)
        end
    end
    ...
    light:SetFalloff(0.7)      --最大范围再向外辐射的光效，1几乎不辐射（衰减）
    light:SetIntensity(.5)     --光照强度.99为强光
    light:SetRadius(0.5)       --光圈范围
    light:SetColour(232/255, 141/255, 67/255)   --光圈颜色
    light:Enable(true)
