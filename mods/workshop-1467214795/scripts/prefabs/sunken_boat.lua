...
local function Land(inst, isday)
    if not CanLand(inst) or not isday then
    ...
    bird.landingtask = bird:DoPeriodicTask(0, function()
        ...
        if newpos.y <= 0.1 then
            ...
            bird:ListenForEvent("animover", function()
                if inst.Land then
                    print(666)
                    inst:StopWatchingWorldState("isday", inst.Land)
                end
...
local function ontimerdone(inst, data)
    if data.name == "land" then
        ...
        if TheWorld.state.issummer and TheWorld:HasTag("island") then
            inst:Land()
        else
            inst.waitingtoland = true
            inst:WatchWorldState("isday", inst.Land)
        end
    end
end
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("parrot_pirate.tex")
    ...
    inst:DoTaskInTime(0, function()
        if inst:HasBird() and TheWorld.state.iswinter then
            inst:TakeOff()
        end
    end)
