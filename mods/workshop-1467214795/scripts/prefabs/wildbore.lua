...
local function OnAttackedByDecidRoot(inst, attacker)
    ...
    local ents = TheSim:FindEntities(x, y, z, SpringCombatMod(SHARE_TARGET_DIST) * .5, SUGGESTTARGET_MUST_TAGS, SUGGESTTARGET_CANT_TAGS) --springcombatmod is handled here
    local num_helpers = 0
...
local CAMPFIRE_TAGS = { "campfire", "fire" } --now they will sleep near fire and campfires like dst pigmen
...
local function common()
    ...
    --Sneaked this into pristine state for optimization
    inst:AddTag("_named")
    inst:AddTag("trader")
...
--Prefab("wildboreguard", guard, assets, prefabs) unused sw enemy 
